(function() {
	...
	function Channel(apiToken) {
		...
		self.emmitError = function(error){
			_.each(handlers['error'], function(handler) {
				handler(error);
			});
		};
			...
			client.onopen = function () {
				...
				fetch.fetchUrl((process.env.PIPEDRIVE_API_PROTOCOL || 'https')+'://'+(process.env.PIPEDRIVE_API_HOST || 'api.pipedrive.com')+'/v1/authorizations/nonce?api_token=' + encodeURIComponent(apiToken), options, function(error, meta, body) {
					...
					try {
						...
						self.emmitError('Could not parse API response' + e);
					}
					...
					if (data && data.nonce) {
						...
						self.emmitError('Authorization failed');
					}
				});
			};
			client.onmessage = function (msg) {
				if (msg && msg.type === 'message') {
					...
					try {
						...
						self.emmitError('Malformed JSON received from socket');
					}
